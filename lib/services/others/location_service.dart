...
class LocationService with ChangeNotifier {
  ...
  setDestination(LatLng? newDestination) async {
    ...
    logger.i('✅setDestination✅ $newDestination $address');
    notifyListeners();
  }
  ...
  animateCameraPosition() {
    if (_mapController != null && _currentPosition != null) {
      ...
      logger.i('marker null hai isiliye animate nahi kiya bhai');
    }
  }
  ...
  Future<List<Placemark>> searchAddress(String query) async {
    try {
      ...
      logger.i('locationFromAddress completed');
  ...
  void handleSearchError(dynamic error) {
    if (error is PlatformException) {
      logger.e('Error searching for address: ${error.message}');
    } else {
      logger.e('Error searching for address: $error');
    }
  }
  ...
  /// Handles address retrieval errors and logs the appropriate e message.
  void handleAddressRetrievalError(dynamic error) {
    if (error is PlatformException) {
      logger.e('Error retrieving address: ${error.message}');
    } else {
      logger.e('Error retrieving address: $error');
    }
  }
  ...
  Future<LatLng?> getUserLocation() async {
    try {
      ...
      setIsLoading(false);
      logger.i('❌ Location services are disabled: $e');
      await Geolocator.openLocationSettings();
    } on GeolocatorPlatform catch (e) {
      setIsLoading(false);
      logger.i('❌ Geolocator platform exception: $e');
    } catch (e) {
      setIsLoading(false);
